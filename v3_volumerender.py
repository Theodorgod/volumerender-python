...
def main():
    ...
    for i in range(Nangles):
        ...
        for dataslice in camera_grid:
            ...
            b = rgba[2]
            a = rgba[3]
            img_r = image[:,:,0] # numexpr does not allow slice of np.array
            img_g = image[:,:,1]
            img_b = image[:,:,2]
            evaluate("a * r + (1 - a) * img_r", out=img_r)
            evaluate("a * g + (1 - a) * img_g", out=img_g)
            evaluate("a * b + (1 - a) * img_b", out=img_b)
